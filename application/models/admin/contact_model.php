...
class Contact_model extends MY_Model {
	...
	function changeStatus($id){
		$contact = $this->get('status',array('id'=>$id));
		$where = NULL;
		if($contact['status'] == 1){
		...
		$this->db->update('utt_customer',$where,array('id' => $id));
	...
	function get($param, $where){
		$this->db->select($param)->from('utt_customer');
		if(isset($where) && count($where)){
			$this->db->where($where);
		}
	...
	function getcount($param=NULL){
		$this->db->select('id')->from('utt_customer');
	...
	function del($param){
		...
		$this->db->delete('utt_customer', $param); 
		$flag = $this->db->affected_rows();
		if($flag>0){
			return array(
				'type'=>'successful',
