...
class Myimage {	   
	   ...
	   function resize($width,$height) {
	      ...
	      $this->image = $new_image;   
	   	...
	   	function resizeFromWidthAndHeight($imgSourcePath, $width, $height, $newImagePath, $jpgQuality = 100)
	   	{
			/*
		...
		function reSizeAndCrop($imgSourcePath, $width, $height, $newImagePath, $jpgQuality = 100)
		{
//            exec("convert -quality 100  -crop $width".'x'."$height+0+0 -colorspace rgb -strip -filter Lanczos -sharpen 0  $imgSourcePath $newImagePath");
//			return true;
			//Get Image size info
		 	$imgInfo = getimagesize($imgSourcePath);
			switch ($imgInfo[2]) {
		...
		function mergeImage($pathDes, $pathSource, $newImagePath, $jpgQuality = 100)
		{
			$imgSrcInfo = getimagesize($pathSource);
			$imgDesInfo = getimagesize($pathDes);
			...
			switch ($imgDesInfo[2]) {
		 	...
		 	$createImage = imagecreatetruecolor($imgSrcInfo[0] + $imgDesInfo[0], $imgSrcInfo[1]);
			if( $imgSrcInfo[2] == 1 || $imgSrcInfo[2]==3 || $imgDesInfo[2] == 1 || $imgDesInfo[2]==3)
			{
   				...
   				imagefilledrectangle($createImage, 0, 0, $imgSrcInfo[0] + $imgDesInfo[0], $imgSrcInfo[1], $transparent);
			}
		 	
		 	imagecopy($createImage, $imDes, 0, 0, 0, 0, $imgDesInfo[0], $imgDesInfo[1]);
		 	imagecopy($createImage, $imSrc,  $imgDesInfo[0], 0, 0, 0, $imgSrcInfo[0], $imgSrcInfo[1]);
			/*
		 	$type = 2;
			...
			switch ($imgSrcInfo[2])
		 	...
		 	switch ($type)
			{
				case 1: imagegif($createImage,$newImagePath); break;
				case 2: imagejpeg($createImage,$newImagePath, $jpgQuality);  break;
				case 3: imagepng($createImage,$newImagePath); break;
				default:  return false;
			}
			*/
			imagejpeg($createImage,$newImagePath, $jpgQuality);
