 ...
 */
class image_CRUD {
	protected $status_feild = NULL;
	protected $status_value = NULL;
	protected $get_photo_where = NULL;
	...
	protected $type_feild = NULL;
	protected $library_view_file = NULL;
	...
	function set_library_view_file($table_name)
	{
		$this->library_view_file = $table_name;
	...
	function set_type_feild($field_name)
	{
		$this->type_feild = $field_name;
		return $this;
	}
	function set_type_feild_value($field_name)
	{
		$this->type_feild_value = $field_name;
		return $this;
	}
	function set_photo_where($field_name)
	{
		$this->get_photo_where = $field_name;
		return $this;
	}

	function set_status_feild($field_name)
	{
		$this->status_feild = $field_name;
		return $this;
	}
	function set_status_value($field_name)
	{
		$this->status_value = $field_name;
		return $this;
	}
	...
	protected function _upload_file($upload_dir) {
		...
		$reg_exp = '/(\\.|\\/)(gif|jpeg|jpg|png|txt|pdf|ppt|doc|docx|xls|xlsx|pptx|rar|zip)$/i';
    ...
    protected function _insert_table($file_name, $relation_id = null)
    {
		...
		$insert[$this->type_feild] = $this->type_feild_value;
		if(!is_null($this->status_feild)){
			$insert[$this->status_feild] = $this->status_value;
		}
    ...
    protected function _delete_file($id)
    {
		...
		if(file_exists($this->image_path.'/'.$result->{$this->url_field})){
			unlink( $this->image_path.'/'.$result->{$this->url_field} );
		}
		if(file_exists($this->image_path.'/'.$this->thumbnail_prefix.$result->{$this->url_field} )){
    ...
    protected function _get_photos($relation_value = null)
    {
		...
		if(is_array($this->get_photo_where)){
			$this->ci->db->where($this->get_photo_where);
		}
		...
		if(!empty($state_info))
		{
			switch ($state_info->name) {
				case 'list':
					...
					$this->_library_view($this->library_view_file,array(
				...
				case 'upload_file':
					...
					if ($file_name !== false) {
						...
						$this->_insert_table($file_name, isset($state_info->relation_value) ? $state_info->relation_value : null );
