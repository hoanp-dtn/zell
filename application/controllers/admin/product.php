...
class Product extends MY_Controller {
	...
	public function del($id = 0){
		...
		$this->session->set_flashdata('message_flashdata',$flag);
		redirect('admin/product/recycle');
	}
	...
	function view($page=1){
		...
		$this->layout->title('Quản lí sản phẩm');
	...
	function add(){
		...
		if(isset($_POST) && !empty($_POST)){
			...
			if($this->form_validation->run()){
				$flag=$this->model_posts->add_product($current_file, 'product');
				$this->session->set_flashdata('message_flashdata',$flag);
				redirect('admin/product/view');
			}
		...
		$image_crud->set_library_view_file('product.php');
		...
		$html .= $this->load->view('backend/product/posts_add',isset($data)?$data:'',true);
		$html .= $this->adminlayout->loadFooter();
		$this->layout->title('Quản lí sản phẩm');	
	...
	function edit($id=0){
		...
		if(isset($_POST) && !empty($_POST)){
			...
			$this->form_validation->set_rules('detail','Nội dung sản phẩm','trim|required');
			...
			$this->form_validation->set_rules('status','trạng thái hiển thị ','callback__getStatus');
			if($this->form_validation->run()){
				$flag=$this->model_posts->edit_pr($id, 'product');
				$this->session->set_flashdata('message_flashdata',$flag);
				redirect($redirect);
			}
			$this->form_validation->set_error_delimiters('<div class="alert alert-warning alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button><h4><i class="icon fa fa-warning"></i> Cảnh báo</h4>','</div>');
		...
		$html .= $this->load->view('backend/product/posts_edit',isset($data)?$data:'',true);
		$html .= $this->adminlayout->loadFooter();
		$this->layout->title('Quản lí sản phẩm');	
		$this->layout->js('publics/admin/plugins/ckeditor/ckeditor.js',true);
		$this->layout->js('publics/admin/plugins/select2/distfd/js/select2.min.js',true);
	...
	function send()
	{
		...
		if (isset($_POST['mailsend'])&&!empty($_POST['mailsend']))
		{
		    ...
		    if ($this->email->send()) {
		        echo json_encode(array("success"=>"sản phẩm '".$title."' đã được gửi thành công.","listsended"=>$arraymail), JSON_PRETTY_PRINT);
		    } else {
		        //show_error($this->email->print_debugger());
		        echo json_encode(array("error"=>"Có lỗi xảy ra! sản phẩm chưa được gửi đi"), JSON_PRETTY_PRINT);
		    }
		}
	}
}
