...
class Gallery extends MY_Controller
{
	...
	function view($page = 1){
		...
		$config['total_rows'] = $this->category_model->total(array('type'=>'gallery'));
		...
		if($config['total_rows']>0){
			$data['gallery'] = $this->category_model->get('utt_cate' ,'utt_cate.title,utt_cate.id',array('type'=>'gallery'),($page*$config['per_page']), $config['per_page']);
		}
	...
	function managerimages($id = 0){
		...
		$getID = $this->category_model->get('utt_cate','utt_cate.id,utt_cate.title',array('id' => (int)$id));
	...
	function edit($id= 0){
		...
		$getID = $this->category_model->get('utt_cate','utt_cate.id',array('id' => (int)$id));
		$gallery = $this->category_model->get('utt_cate' ,'utt_cate.title,utt_cate.id',array('type'=>'gallery','id'=>(int)$id));
		if(!isset($getID)||count($getID)==0){
			redirect('admin/gallery/view');
		}
		if(isset($_POST) && !empty($_POST)){
