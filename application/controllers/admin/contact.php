...
class Contact extends MY_Controller {
	...
	function __construct() {
		...
		(isset($this->redirect) && !empty($this->redirect))?($this->redirect=base64_decode($this->redirect)):($this->redirect='admin/contact/view');
	...
	function view(){
		...
		$config['base_url']	= $this->config->base_url('admin/contact/view/'.$this->lang_code);

		$config['total_rows'] = $this->contact_model->getcount();
		...
		if($config['total_rows']>0){
			...
			$data['contact'] = $this->contact_model->view(
	...
	function del($id = 0){
		...
		$contact = $this->contact_model->get('id',array('id'=> (int)$id));

		if(!isset($contact) || count($contact)==0){

			$this->session->set_flashdata('message_flashdata',array('type'=>'error','message'=>'contact này không tồn tại'));

			redirect($this->redirect);

		}

		$flag = $this->contact_model->del(array('id' => $id));
	...
	function changeStatus($id=0){
		...
		$flag = $this->contact_model->changeStatus($id);
	...
	public function edit($id = 0){
		...
		$contact = $this->contact_model->view(

			array('*','utt_contact.id as contact_id','utt_contact.status as o_status'),
		...
		if(isset($_POST)&&!empty($_POST)){
			...
			$data['current_post'] = $this->Model_posts->get('id, title', array('post_type'=>'news','lang'=>$contact['lang'],'id'=>(int)$post_id),false);
			...
			if($this->form_validation->run()){
				...
				$flag = $this->contact_model->change($id);
		...
		$data['list_location'] = $this->contact_model->dropdown(array('lang'=>$contact['lang']));

		$html  = $this->adminlayout->loadTop();

		$html .= $this->adminlayout->loadMenu();

		$html .= $this->load->view($this->template_f . 'backend/contact/contact_edit', isset($data)?$data:'', true);
	...
	function add(){
		...
		$data['active'] = array('contact','contact/add');

		$data['lang'] = $this->lang_code;

		$data['list_location'] = $this->contact_model->dropdown(array('lang'=>$this->lang_code));

		$html  = $this->adminlayout->loadTop();

		$html .= $this->adminlayout->loadMenu($current = 'treeview', $viewFile = 'backend/admin/menu_view',  $data);

		$html .= $this->load->view($this->template_f . 'backend/contact/contact_add', isset($data)?$data:'', true);

		$html .= $this->adminlayout->loadFooter();

		$this->layout->title('Thêm contact');		
	...
	function _checkLocation($val){
		...
		$count = $this->contact_model->getcount(array());
	...
	public function getLocation(){
		...
		$list_location =$this->contact_model->dropdown(array('lang'=>(isset($lang) && !is_bool($lang))?$lang:'vn'));
