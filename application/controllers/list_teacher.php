...
class List_teacher extends MY_Controller {
    private $data;
    function List_teacher() {
        ...
        $this->load->model('teacher_home_model');
    ...
    function index(){
       $this->setInformationSite($data);
		...
		$listTeacher = $this->teacher_home_model->getListTeacher();
        $html .= $this->render('home/list_teacher',array('listTeacher'=>$listTeacher,'dataDepartment'=>$dataDepartment,'langCode'=>$langCode,'getPostAndNew' => $getPostAndNew,'dataAds' => $dataAds, 'dataPartner' => $dataPartner,'dataGallery' => $dataGallery),true
        );
